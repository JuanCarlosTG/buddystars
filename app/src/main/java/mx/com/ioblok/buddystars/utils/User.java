...
public class User {
    ...
    static public void logged(Boolean value, Activity a, String token) {
        SharedPreferences.Editor e = PreferenceManager.getDefaultSharedPreferences(a).edit();
        if(value){
            e.putString("profile_logged", "true");
    ...
    static public boolean can(String permission, Activity a) {

        Map<String,?> data = PreferenceManager.getDefaultSharedPreferences(a).getAll();
        String result = (String)data.get("profile_permissions");
        try {
            JSONObject permissions = new JSONObject(result);
